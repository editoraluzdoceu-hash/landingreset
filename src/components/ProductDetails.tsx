...
import { delivery, official, productTabs, quizMoments, toolNames, type ProductTab } from '../data/content';
...
export default function ProductDetails({ tab }: { tab: ProductTab }) {
  ...
  return (
    <div className="legal-content product-details">
      <ResetMark />
      <p className="eyebrow">POR DENTRO DO KIT RESET</p>
      <h3>{product.title}</h3>
      ...
      {tab === 'plan' && <>
        <h4>Um cronograma por semanas</h4>
        <p>Na primeira semana, reconheça a queda e comece a estabilizar. Nas semanas 2 e 3, trabalhe suas bases. Ao final da terceira semana, repita a Escala de Prontidão e escolha um hábito, se estiver pronto. Na quarta, sustente esse hábito, revise e retome quando necessário.</p>
      ...
      <div className="details-purchase">
        <CheckoutLink>Quero o Kit RESET completo</CheckoutLink>
        <p>Pagamento único. Acesso vitalício. Garantia de {official.guaranteeDays} dias.</p>
